...
# The flags are googl's way of handling command line parameters.
FLAGS = tf.app.flags.FLAGS

tf.app.flags.DEFINE_string(
    "train_dir", "flickr_logos_27_dataset",
    "Directory where to write event logs and checkpoint.")
tf.app.flags.DEFINE_integer("max_steps", 1000, "Number of batches to run.") # Model start to overfit the data and the performance decreases of you want for better performance choose more number of Images in Ipython Notebook.
...
# reshaping images for our neural network
...
# reading data into train, test, validation sets
def read_data():
...
# Creating convolutional tensorflow model             
# DATA    -> (conv_layer -> pool)-> (conv_layer -> pool) -> (conv_layer -> pool) -> fully connected_layer ->  Output_layer
# using foumula -> Wx + b
...
def main():
    ...
    with graph.as_default():
        # truncated Variables from a normal distribution with standard deviation of 0.1
        w_conv1 = tf.Variable(
            tf.truncated_normal(
        ...
        # Parameters of NN weights and baises
        params = [
        ...
        # Tensorboard namescopes 
        # refer -> https://www.tensorflow.org/get_started/summaries_and_tensorboard
    ...
    with tf.Session(graph=graph) as session:
        ...
        for step in range(FLAGS.max_steps):
            ...
            # data into batches its a 4d input
            ...
            try:
                ...
                if step % 50 == 0:
                	# on evert 100th step ouptput of the NN will save to the log file foe tensorboard
